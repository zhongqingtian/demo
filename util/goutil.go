...
func SessionId() string {
	b := make([]byte, 32)
	temp := make([]byte, 32)
	//ReadFull 从rand.Reader精准地读取len(b)字节数据填充进b
	//rand.Reader是一个全局、共享的密码用强随机数生成器
	n, _ := rand.Read(temp)
	fmt.Println(n, base64.URLEncoding.EncodeToString(temp))
	if _, err := io.ReadFull(rand.Reader, b); err != nil {
		return ""
	}
	fmt.Println(b)
...
//将interface转换为string类型
func GetOrderId(mapStrI map[string]interface{}) string {
	if len(mapStrI) == 0 {
		return ""
	}
	mapStrStr := make(map[string]string)
	for k, v := range mapStrI {
	...
	return buildSign(mapStrStr)
}

//按照参数名进行排序，末尾加秘钥，结果为16进制字符集字符串
...
func getTableName(userID uint64) string {
	...
	return fmt.Sprintf("%s%d", sendGiftLogTablePrefix, hashID)
}
