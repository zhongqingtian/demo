...
func Detail(w http.ResponseWriter, r *http.Request) {
	...
	redisData, _ := client.HGet("demo", sid).Result()
	...
	if redisData == "" { //判断是否为空，如果空，说明redis中没有数据，则从数据库中查询
		...
		bor, err := client.HSet("demo", sid, tjson).Result() //存储在redis
		fmt.Println(bor, err)
		w.Write(tjson)
	}
...
func List(w http.ResponseWriter, r *http.Request) {
	...
	w.Write(wjson)
	//批量生成 hset
	client.HMSet("demo", cacheMap)
	//tjson,_ := json.Marshal(dataMap)
	//bcm := client.HSet("demo",tjson)
	// -1 表示永远有效
	//client.Set("demo",tjson,-1)
	//
	//client.HSet("demo","all",tjson)
...
func ExampleClient() {
	//fmt.Println(client.HSet("key2","lj","kdkdk"))
	//存储值再redis中
	...
	val2, err := client.HGet("key2", "lj").Result()
	if err == redis.Nil {
